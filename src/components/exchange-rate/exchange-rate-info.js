...
export const ExchangeRateInfo = ({symbol}) => {
    ...
    const generateSymbolInfo = () => {
        return(<>
            <img  src={symbolInfo.Flag} width="30px" alt={symbolInfo.CountryName} style={{marginRight: "10px"}} />
            {symbolInfo.Code} - {symbolInfo.CountryName} - {symbolInfo.Symbol}
        </>);
    }

    const generateDefaultSymbol = () => {
        return(<Grid container spacing={1.8}>
            <Grid item>
                <ImageNotSupported sx={{marginLeft: .6, fontSize: 20}} />
            </Grid>
            <Grid item >
                {symbol}
            </Grid>
    ...
    return(<>
        { symbolInfo ? generateSymbolInfo() : generateDefaultSymbol()}
    </>)
}