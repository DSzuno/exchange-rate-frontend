...
export const ExchangeRateList = ({
  ...
  return (
    <>
      ...
      {!isLoading && error && (
        <EmptyListCard
          message={error === true ? "Error while try to load data!" : error}
          icon={<ErrorOutline sx={{ fontSize: 100 }} color="error" />}
        />
      )}
      {!isLoading &&
        rateList &&
        getFilteredList().length === 0 &&
        searchText !== "" && <EmptyListCard />}
    </>
  );
};
